...
const TutorialDetails = ({ tutorial, error }) => {
  ...
  const deleteTutorial = async () => {
    try {
      await fetch(`https://suspicious-volhard-e36ba7.netlify.app/api/tutorials/${tutorial._id}`, {
        method: "DELETE",
      });
    } catch (error) {
      console.log(error);
...
export async function getServerSideProps({ query: { id } }) {
  const res = await fetch(`https://suspicious-volhard-e36ba7.netlify.app/api/tutorials/${id}`);
  if (res.status === 200) {
